...
function Gingerbread(){
  ...
  this.drawGingerbread = (m, drawShape, headPosition, orientation, leftController, rightController) => {
    ...
    m.save();
      ...
      let y_hand_angle = 0;
      m.save();
        m.translate(-0.22, -0.4, 0.08);
        // drawLimb(m, drawShape, toRad(180), toRad(0),toRad(180), true);
        drawLimb(m, drawShape, 0, toDeg(leftController.orientation[1]),toDeg(leftController.orientation[2]), true);
      m.restore();

      m.save();
        m.translate(0.22, -0.4, 0.08);
        // drawLimb(m, drawShape, toRad(180), toRad(0),toRad(0), false);
      ...
      m.save();
        ...
        m.translate(-.19, -1, 0.013);
        drawLegs(m, drawShape, toDeg(walk_angle), Math.PI/2,Math.PI);
      m.restore();

      // // //left hip
      m.save();
        m.translate(.19, -1, 0.013);
        drawLegs(m, drawShape, -1*toDeg(walk_angle), Math.PI/2,Math.PI);
      m.restore();
...
function drawTorso(m, drawShape){
  ...
  m.save();
    ...
    m.translate(0,0.05,-.05);
    m.scale(1.2, 1.4, 2.6);
    drawShape(CG.body, [1, 1, 1], GINGERBREAD_TEXTURE);
  m.restore();
}
...
function drawLimb(m, drawShape, x_rotation, y_rotation, z_rotation, is_left_limb){
    ...
    m.save();
      ...
      let arms_length = 0.4;
      m.scale(arms_length/15, arms_length, THICKNESS);
...
function drawLegs(m, drawShape, x_rotation, y_rotation, z_rotation){
  ...
  m.save();
  m.translate(0,0.02,.07);    
    m.rotateX(Math.PI);
    m.rotateX(toRad(10));
    m.rotateY(y_rotation);
    m.rotateZ(z_rotation);
    // m.scale(.03, .6, .03);
    drawShape(CG.leg,[1,1,1], GINGERBREAD_TEXTURE);
  m.restore();
}
